...
#include "utils/SARMA/include/sarma.hpp"
#include "utils/SARMA/include/tools/utils.hpp"
#include "utils/SARMA/include/algorithms/nicol1d.hpp"
#include "utils/PIGO/pigo.hpp"
...
namespace sarma {
    ...
    public:
        ...
        **/
        Matrix(const ns_filesystem::path &path, bool save_binary=true, const bool use_data = false) {
            if (!ns_filesystem::exists(path)) {
                ...
                pigo::CSR<Ordinal, Ordinal, std::vector<Ordinal>, std::vector<Ordinal>> g { path };

                indptr_ = std::move(g.offsets());
                indices_ = std::move(g.endpoints());
                M_ = g.ncols();

                while (indptr_.size() < g.nrows() + 1){
                    indptr_.push_back(indptr_.back());
                }

                this->use_data(use_data);
            }

            std::cerr << "# Graph read.. N: " << N() << ", NNZ: " << NNZ() << std::endl;
        }
        ...
        Matrix(const std::vector<std::pair<Ordinal, Ordinal>> &pts, const std::vector<Value> &data, bool _use_data = false) : sorted_(true) {
            ...
            for (size_t i=0; i<indptr_.size()-1; i++){
                indptr_[i] = std::lower_bound(std::begin(idx), std::end(idx), i, [&](const auto &idx_i, const auto &i) {
        ...
        **/
        Matrix transpose() const {
            ...
            for (size_t tid=0; tid<v_arr.size()-1; tid++){
                for (Ordinal i = v_arr[tid]; i < v_arr[tid + 1]; i++){
        ...
        Matrix & sort() {
            if (!sorted) {
                #pragma omp parallel for schedule (dynamic, 1024)
                for (size_t i=0; i<indptr.size()-1; i++){
                    std::sort(indices_.begin() + indptr[i], indices_.begin() + indptr[i + 1]);
                }
                sorted_ = true;
            }
            return *this;
        ...
        **/
        Matrix order(Order order = Order::NAT, bool triangular = false, bool reverse = false, int seed = 1) const {
            ...
            auto get_order_map = [&] () {
                ...
                std::vector<std::pair<Ordinal, Ordinal>> degrees(N());
                
                #pragma omp parallel for schedule(dynamic, 1024)
                for (size_t i = 0; i < N(); i++){
                    // degrees.push_back({i, indptr[i + 1] - indptr[i]});
                    degrees[i] = {i, indptr[i + 1] - indptr[i]};
                }
