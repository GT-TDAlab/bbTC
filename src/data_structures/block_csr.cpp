...
template <class Ordinal, class Vertex>
BlockCSR<Ordinal, Vertex>::BlockCSR(const std::vector<Vertex> &cuts, const std::vector<Ordinal>& x_adj, const std::vector<Vertex>& adj)
{
    P_ = cuts.size() - 1;
    NoOfVertices = x_adj.size()-1;
    NoOfEdges = adj.size();
    CSRtoBCSR(cuts, x_adj, adj);
    ComputeTileProps();
}
...
template <class Ordinal, class Vertex>
void BlockCSR<Ordinal, Vertex>::CSRtoBCSR(const std::vector<Vertex> &cuts, const std::vector<Ordinal>& x_adj, const std::vector<Vertex>& adj)
{
    P_ = cuts.size() - 1;
    size_t n_tiles = P_ * P_ + 1;
    size_t n_rpart = (NoOfVertices + P_) * P_;
